...
class OpenAISingleton():
    ...
    @classmethod
    async def create_completion_message(cls, message: str):
        """
        This method create a new message in the completions api
        """

        completion = await cls.__client.chat.completions.create(
            model=settings.OPENAI_MODEL,
            messages=[
                {"role": "system", "content": """Lo siguiente es una conversación entre un usuario y un asistente de fraudes, estafas y extorsiones, debes de
resumirla y destacar los puntos más importantes, de igual modo dar consejos, recomendaciones y pasos posteriores
que debe hacer
"""},
        ...
        return {'data': {'msg': completion.choices[0].message.content}, 'status_code': HTTPStatus.OK}
